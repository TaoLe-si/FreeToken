...
for i, eid in enumerate(ids):
    ...
    gg = read_raw(f"gate_up_global#L00000").reshape(NE, 2*I).view(np.uint16) if False else None
    # gate_up_global 名字确认
    gg_name = f"gate_up_global#L00000"
    if gg_name not in entries: gg_name = f"gate_up_global_scale#L00000"
    gg = read_raw(gg_name).view(np.uint16).reshape(NE, 2*I)
    gu = dequant(gp, gs, f16(gg[eid]), 2*I, H) @ hidden
    gate, up = gu[:I], gu[I:]
    ...
    dg = read_raw(f"down_global#L00000").view(np.uint16).reshape(NE, H)
    out_ref += weights[i] * (dequant(dp, ds, f16(dg[eid]), H, I) @ act)

print("dll out[0,:4]:", dll_out[:4])
print("ref out[0,:4]:", out_ref[:4])
print("dll norm:", np.linalg.norm(dll_out), "ref norm:", np.linalg.norm(out_ref))
rel = np.abs(dll_out - out_ref) / (np.abs(out_ref) + 1e-9)
print("max rel:", rel.max(), "median:", np.median(rel))
# 检查 bank 非零
gp0 = read_raw("gate_up_packed#L00000")
print("bank nonzero ratio:", (gp0 != 0).mean())
