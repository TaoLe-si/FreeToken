...
import importlib.util, numpy as np, torch, os, time
...
print("DLL decode: %.2fs out=%s finite=%s norm=%.2f" % (dt, tuple(out.shape), bool(torch.isfinite(out).all()), float(out.norm())))
# 真实模型规模基准测试
...
print("DLL M=4096 K=4096 single: %.2fms" % ((time.perf_counter()-t0)*1000))
svc.close()
print("ALL OK")
