...
int main() {
    std::cout << "Hello D3D12 v2" << std::endl;
    ...
    device->CreateCommittedResource(&hpDef, D3D12_HEAP_FLAG_NONE,
        &make_buffer_desc(128 * 4, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS),
    ...
    device->CreateCommittedResource(&hpRb, D3D12_HEAP_FLAG_NONE,
        &make_buffer_desc(128 * 4, D3D12_RESOURCE_FLAG_NONE),
    ...
    for (int i = 0; i < 8; i++) {
        std::cout << "outv[" << i << "] = " << ((float*)om)[i] << std::endl;
    }
    return 0;
}
