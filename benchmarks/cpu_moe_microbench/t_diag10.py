...
variants = [
    ...
    ('C: real P, zero S, zero B, real A', fcW_real.tobytes(), act_real.view(np.int32).tobytes(), bytes([0]*512), bytes([0]*512)),
    ('D: real P, zero S, zero B, rand A', fcW_real.tobytes(), (np.random.randn(fk)*0.1).astype(np.float32).view(np.int32).tobytes(), bytes([0]*512), bytes([0]*512)),
    ('E: SAFETENSORS fcW[0], real A', fcW[0].numpy().tobytes(), act_real.view(np.int32).tobytes(), bytes([0]*512), bytes([0]*512)),
]
