...
import sys
sys.stderr.write('err: ' + err.decode(errors='replace')[:500] + '\n')
sys.stdout.flush()
if len(out) >= 4:
    sz = struct.unpack('<I', out[:4])[0]
    if sz > 0 and sz < 100000:
        v = np.frombuffer(out[4:4+sz], dtype=np.float32).copy()[0]
        sys.stdout.write(f'FC a=0.05: {v} (expected 0.271919)\n')
        sys.stdout.write(f'Match: {abs(v - 0.271919) < 1e-4}\n')
    else:
        sys.stdout.write(f'bad sz: {sz}\n')
sys.stdout.flush()
