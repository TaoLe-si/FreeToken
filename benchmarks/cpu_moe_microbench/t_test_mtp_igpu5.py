...
# Wrap igpu_fc with bf16->fp32 adapter
fc_packed_1 = head._packed_mxfp4['fc.weight'][0:1].numpy().astype(np.uint32)
class IgpuFcAdapter:
    def __init__(self, sticky):
        self.sticky = sticky
    def __call__(self, act_bf16_flat):
        # act_bf16_flat is bf16 from torch.cat([emb_n, hid_n]).view(-1)
        # The MTP head code: cat_flat = cat.view(-1).to(torch.float32)
        # But it expects fc to take fp32. Let's see if it's already fp32 or bf16
        if act_bf16_flat.dtype == torch.bfloat16:
            act_fp32 = act_bf16_flat.to(torch.float32)
        else:
            act_fp32 = act_bf16_flat
        act_np = act_fp32.cpu().numpy().astype(np.float32)
        outv = self.sticky(act_np)
        # outv is [1] float32, return torch tensor
        return torch.from_numpy(outv.copy())
