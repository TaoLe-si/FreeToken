"""NVFP4 reference using REAL bias+scale (matches shader formula)."""
...
print(f"\nref[:5] = {outv[:5]}")
print(f"D3D12[:5] = {d3d[:5]}")

finite = np.isfinite(d3d) & (np.abs(d3d) < 1e5)
diff = d3d[finite] - outv[finite]
print(f"\nmax abs diff = {np.abs(diff).max():.4e}")
print(f"mean abs diff = {np.abs(diff).mean():.4e}")
rel = np.abs(diff) / (np.abs(outv[finite]) + 1e-6)
print(f"max rel diff = {rel.max():.4e}")
print(f"% finite = {finite.sum()}/{len(d3d)}")
