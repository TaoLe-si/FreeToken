...
class OffloadMoeCache:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.decode_target in ("gpu", "cpu", "hybrid", "igpu"), self.decode_target
        assert self.quant_format in _BANK_SCHEMAS, f"unknown quant_format {self.quant_format!r}"
        # Attached by the engine for decode_target == "cpu" (CpuMoeExecutor) or
    ...
    def set_igpu_executor(self, executor) -> None:
        """Attach the iGPU D3D12 executor (decode_target == "igpu").

        The executor owns the persistent D3D12 GEMV service process and reads the
        native nvfp4 host banks straight from this cache's bank_sources.
        """
    ...
    def is_igpu_layer(self, layer_id: int) -> bool:
        """Whether ``layer_id`` decodes on the iGPU (D3D12) executor. True only when
        decode_target == "igpu" (all MoE layers); the CPU layer set is empty in that
        mode so the CPU branch never claims a layer."""
        return self.decode_target == "igpu"
